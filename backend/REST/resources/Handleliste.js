...
// Hent en fullstendig handleliste
  server.get('rest/handleliste/:handleliste_id',function(req, res, next){
    connection.connection.query("SELECT * FROM Handleliste WHERE handleliste_id=?", [req.params.handleliste_id], function(err, rows, fields){
      ...
      connection.connection.query("SELECT Vare.* FROM Vare " +
...
// Post en fullstendig handleliste
  server.post('rest/handleliste/',function(req,res,next){
    ...
    connection.connection.query('INSERT INTO Handleliste SET ?', [handleliste], function(err,rows,fields){
      if(err)
        return next(err);
      ...
      connection.connection.query('INSERT INTO Vare (navn, handleliste_id, antall) VALUES ?', [varer], function(err,rows,fields){
        if(err)
          return next(err);
        res.send(rows);
      });
...
// Hent handlelister for en bestemt undergruppe
  server.get('rest/handlelisteForUndergruppe/:undergruppe_id',function(req, res, next) {
    connection.connection.query("SELECT * FROM Handleliste WHERE undergruppe_id=?", [req.params.undergruppe_id], function (err, rows, fields) {
...
// Oppdater en handleliste
  server.put('res/handleliste/', function (req,res,next) {
    ...
    */
    ...
    connection.connection.query('UPDATE Handleliste SET ? WHERE handleliste_id=?', [req.body, req.body.handleliste_id], function (err,rows,fields) {
      if(err)
        return next(err);
      res.send(rows);
      return next();
      /*
      connection.connection.query('SELECT * FROM Vare WHERE handleliste_id=?', req.body.handleliste_id, function (err, rows, fields) {
        if(err)
          return next(err);
        let varerDelete = [];
        let varerUpdate = [];
        ...
        connection.connection.query('UPDATE Vare SET ? WHERE ?', [varerUpdate, varerId], function (err, rows, fields) {
          if(err)
            return next(err);
          res.send(rows);
          return next();
...
// Slett en liste
  server.del('rest/handleliste/:handleliste_id', function (req ,res, next) {
    connection.connection.query('DELETE FROM Vare WHERE handleliste_id=?', req.params.handleliste_id, function (err, rows, fields) {
      if(err)
        return next(err);
      connection.connection.query('DELETE FROM Handleliste WHERE handleliste_id=?', req.params.handleliste_id, function (err, rows, fields) {
        if(err)
          return next(err);
        res.send(rows);
        return next();
...
// Favorittiser en handleliste
  server.put('rest/favorittHandleliste/', function (req, res, next) {
    ...
    connection.connection.query('UPDATE Handleliste SET favoritt=? WHERE handleliste_id=?', [req.body.favoritt, req.body.handleliste_id], function (err, rows, fields) {
      if(err)
        return next(err);
      res.send(rows);
      return next();
