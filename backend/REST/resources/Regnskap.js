...
server.get('rest/regnskap/:regnskap_id', function(req,res,next){
  connection.query('SELECT * FROM Regnskap WHERE regnskap_id=?', [req.params.regnskap_id], function(err,rows,fields){
    if(err || rows.length != 1)
      next(err);

    let regnskap = rows[0];

    connection.query('SELECT Kostnad* FROM Kostnad WHERE regnskap_id=?', [req.params.regnskap_id], function(err,rows,fields){
      if(err)
        return next(err);
...
server.get('rest/regnskap/:undergtuppe_id', function(req,res,next){
  connection.query('SELECT * FROM Regnskap WHERE undergruppe_id=?', [req.params.undergruppe_id], function (err,rows,fields) {
    if (err)
      return next(err);
    let regnskaper = [rows.length];
    for (regnskap in rows) {
      if('opprettet' in regnskap)
        regnskap.opprettet = new Date(regnskap.opprettet);
    }
    connection.query('',[req.params.undergruppe_id, function (err,rows,fields) {
      let regnskaper = [rows.length];
