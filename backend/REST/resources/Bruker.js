...
module.exports = function(connection, server) {
  ...
  server.get('rest/bruker/:bruker_id', function (req, res, next) {
    connection_prod.connection.query("SELECT * FROM Bruker WHERE bruker_id=?", [req.params.bruker_id], function (err, rows, fields) {
...
// Finn bruker med bestemt epost
  server.get('rest/brukerMedEpost/:epost', function (req, res, next) {
    ...
    connection_prod.connection.query("SELECT * FROM Bruker WHERE epost=?", [req.params.epost], function (err, rows, fields) {
      res.send(err ? err : (rows.length == 1 ? rows[0] : null));
      return next();
    });
  });
...
//  console.log('---------------------------------------------------------------');
    ...
    connection_prod.connection.query("INSERT INTO Bruker SET ?", req.body, function (err, rows, fields) {
...
// Hent alle brukere
  server.get('rest/bruker/', function (req, res, next) {
    connection_prod.connection.query("SELECT * FROM Bruker ORDER BY bruker_id ASC", function (err, rows, fields) {
...
// Hent alle brukere i et bestemt kollektiv
  server.get('rest/brukereIKollektiv/:kollektiv_id', function (req, res, next) {
    connection_prod.connection.query("SELECT Bruker.* FROM Bruker " +
...
// Oppdater en bruker
  server.put('rest/bruker', function (req, res, next) {
    ...
    connection_prod.connection.query("UPDATE Bruker SET ? WHERE bruker_id=?", [req.body, req.body.bruker_id], function (err, rows, fields) {
      res.send(err ? err : rows);
      return next();
    });
  });
...
//Check password
  server.post('rest/password', function (req, res, next) {
    connection_prod.connection.query("SELECT * FROM Bruker WHERE epost=?", [req.body.email], function (err, rows, fields) {
      if (err)
        return next(err);
...
//Change password
  server.put('rest/changePassword', function (req, res, next) {
    ...
    connection_prod.connection.query('UPDATE Bruker SET hashed_passord=? WHERE epost=?', [hash, req.body.email], function (err, rows, field) {
      if (err) {
        isUpdated = false;
        return next(err);
      }
