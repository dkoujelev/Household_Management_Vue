...
module.exports = function(asdf, server) {
  ...
  server.get('rest/innmelding/', function (req, res, next) {
    ...
    connection_prod.connection.query("SELECT * FROM Innmelding", function (err, rows, fields) {
...
// Hent alle innmeldinger
  server.get('rest/innmeldingerForKollektiv/:kollektiv_id', function (req, res, next) {
    ...
    //[{"bruker_id":1,"kollektiv_id":1,"status_admin":1,"status_bruker":9,"dato_svar_admin":"2018-01-15T11:12:12.000Z","dato_svar_bruker":null,"aktiv":1,"notat_admin":"Kommer flyttende i mars.","notat_bruker":null},
    // {"bruker_id":2,"kollektiv_id":1,"status_admin":1,"status_bruker":1,"dato_svar_admin":"2018-01-12T09:21:00.000Z","dato_svar_bruker":"2018-01-12T13:22:20.000Z","aktiv":0,"notat_admin":null,"notat_bruker":null}]
    connection_prod.connection.query("SELECT * FROM Innmelding WHERE aktiv=true AND status_bruker=1 AND kollektiv_id=?", req.params.kollektiv_id, function (err, rows, fields) {
      res.send(err ? err : rows);
      return next();
    });
  });
...
// Hent en bestemt innmelding
  server.get('rest/innmelding/:kollektiv_id', function (req, res, next) {
    ...
    if (req.params.kollektiv_id) {
      connection_prod.connection.query("SELECT * FROM Innmelding WHERE bruker_id=? AND kollektiv_id=?", [req.params.bruker_id, req.params.kollektiv_id], function (err, rows, fields) {
...
// Legg til innmelding
  server.post('rest/innmelding/', function (req, res, next) {
    console.log('DEBUG - rest/innmelding/');
    connection_prod.connection.query("INSERT INTO Innmelding SET ?", req.body, function (err, rows, fields) {
      res.send(err ? err : rows);
      return next();
    });
  });
...
// console.log(req.params);
    ...
    if (req.params.kollektiv_id) {
      connection_prod.connection.query("SELECT bruker_id FROM Bruker WHERE epost=?", [req.params.bruker_epost], function (err, rows, fields) {
        req.params.bruker_id = rows[0].bruker_id;
        connection_prod.connection.query("SELECT * FROM Innmelding WHERE bruker_epost=? AND kollektiv_id=?", [req.params.bruker_epost, req.params.kollektiv_id], function (err, rows1, fields) {
          ...
          if (req.params.kollektiv_id) {
            connection_prod.connection.query("UPDATE Innmelding SET ? WHERE kollektiv_id=? AND bruker_epost=?", [minInnmelding, req.body.kollektiv_id, minInnmelding.bruker_epost], function (err, rows2, fields) {
              connection_prod.connection.query("INSERT INTO Bruker_Kollektiv SET kollektiv_id=?, bruker_id=?, er_admin=0", [req.body.kollektiv_id, req.params.bruker_id], function (err, rows3, fields) {
...
// Oppdatere en innmelding via GET (for å kunne motta svar via klikk fra epost)
  server.get('rest/invitasjon/:kollektiv_id', function (req, res, next) {
    ...
    if (req.params.bruker_epost && req.params.bruker_svar == 'jatakk') {
      ...
      connection_prod.connection.query("SELECT undergruppe_id FROM Undergruppe WHERE kollektiv_id=? AND default_gruppe=1", [myKollektiv_id], function (err, rows0, fields) {
      ...
      connection_prod.connection.query("SELECT bruker_id FROM Bruker WHERE epost=?", [myBruker_epost], function (err, rows, fields) {
        if (rows != '') {
          ...
          connection_prod.connection.query("SELECT * FROM Innmelding WHERE epost=? AND kollektiv_id=?", [myBruker_epost, myKollektiv_id], function (err, rows, fields) {
            console.log('* - Rows: ' + rows);
            if (rows.length == 1) {
              //We have the invite. Update it and add the user!
              connection_prod.connection.query("UPDATE Innmelding SET status_bruker=1, dato_svar_bruker=? WHERE bruker_epost=? AND kollektiv_id=?", [currentDateAsEpoch, myBruker_epost, myKollektiv_id], function (err, rows1, fields) {
                console.log('Rows1: ' + rows1);
                connection_prod.connection.query('INSERT INTO Bruker_Kollektiv SET bruker_id=?, kollektiv_id=?', [myBruker_id, myKollektiv_id], function (err, rows2, fields) {
                  console.log('Rows2: ' + rows2);
                  connection_prod.connection.query('INSERT INTO Bruker_Undergruppe SET bruker_id=?, undergruppe_id=?', [myBruker_id, myUndergruppe_id], function (err, rows3, fields) {
